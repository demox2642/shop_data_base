...
interface SaleDao {
    ...
    @Query("SELECT S.id AS id , S.date as date,-- p.name as product_name, \n" +
    ...
    @Query("SELECT W.id AS id, P.name AS product_name, SP.price AS price, W.count AS count_product,SP.id AS price_id,p.id AS product_id\n" +
            "FROM WAREHOUSE W\n" +
