...
interface SupplyDao {
    ...
    @Query(" SELECT S.id AS id, S.date AS date, SU.id AS supplyers_Id, SU.name AS supplyers_name,  \n" +
    ...
    @Query(" SELECT  s.id,s.date, sum(sd.count *sp.price) as count_product , sum(sd.count) as price_summ\n" +
            "             FROM SUPPLY S \n" +
            "             LEFT JOIN SUPPLY_DETAIL SD ON S.id=SD.supply_id \n" +
            "             LEFT JOIN SUPPLIERS_PRICE SP ON SD.supply_price_id=SP.id  \n" +
            "             LEFT JOIN SUPPLIERS SU ON SP.suppliers_id=SU.id \n" +
